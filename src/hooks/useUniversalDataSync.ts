...
function createSafeUseUniversalDataSync() {
  try {
    return function useUniversalDataSync(): UniversalSyncState &
      ...
      const [state, setState] = useState<UniversalSyncState>(() => {
        try {
          ...
          console.error(
            "❌ Error initializing useUniversalDataSync state:",
            error,
          );
          return {
            obras: [],
      ...
      const addObra = useCallback(async (obraData: any): Promise<string> => {
        try {
          ...
          const fallbackId =
            obraData.id || `obra-${Date.now()}-${Math.random()}`;
    ...
    console.error("❌ Error creating useUniversalDataSync:", error);
    // Return a safe fallback hook
    return function useUniversalDataSyncFallback(): UniversalSyncState &
      UniversalSyncActions {
      const [state] = useState<UniversalSyncState>({
        obras: [],
        manutencoes: [],
        piscinas: [],
        clientes: [],
...
export const useUniversalDataSync = createSafeUseUniversalDataSync();
