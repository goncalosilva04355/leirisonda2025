...
export class ErrorBoundary extends Component<Props, State> {
  ...
  public static getDerivedStateFromError(error: Error): State {
    // Apenas capturar erros críticos - deixar outros passarem
    if (
      error.message?.includes("ChunkLoadError") ||
      error.message?.includes("Loading chunk") ||
      error.message?.includes("NetworkError")
    ) {
      console.log("🚨 Critical error caught:", error.message);
      return { hasError: true, error, retryCount: 0 };
    }

    // Para outros erros, não mostrar error boundary - deixar a aplicação continuar
    console.warn("⚠️ Non-critical error ignored:", error.message);
    throw error; // Re-throw para não capturar o erro
  }
